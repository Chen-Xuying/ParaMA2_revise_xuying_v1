...
class SubChange(_StemChange):
    ...
    def __init__(self, ch0, ch1, pos):
        ...
        self.__ch0 = ch0
        self.__ch1 = ch1
        self.__pos = pos  # stem_change_position: right or left boundary. 'r' / 'l'
        if self.__pos == StemChangePosition.RIGHT_BOUNDARY:
            self.apply = lambda x: self.apply_sub(x, self.__ch0, self.__ch1, len(x)-1)  # root[:indx] + ch1 + root[indx+1:] except Exception
...
class NonChange(_StemChange):
    '''
    Define a NonChange Class with the following properties in __init__.
    '''
    def __init__(self):
        '''
        '''
        self.__type = StemChangeType.NON # 公共属性，调来 readable 的 para
        self.__pos = None
        self.__ch = ''
        self.__key = '$'

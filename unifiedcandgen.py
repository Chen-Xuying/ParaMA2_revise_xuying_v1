...
class UniCandGen():
    ...
    def __init__(self, lang, morph_typology, params, lexicon, prefs={}, sufs={}, infs={}):
        ...
        self.__reduplication = morph_typology.has_morph_feature(MorphType.REDUPLICATION) # morph_feature 是一个字典，取键reduplication 的值--True
...
#         pass
    ...
    def __find_cand(self, cand_pats, cond=lambda x: False): # 在 cand_pats_list 里，if cond 满足 cond， 则 T
    ...
    def get_candidate_analyses(self, word):
        cand_pats = []
        if len(word) <= self.__min_stem_len: # 默认是3
        ...
        if self.__do_reduplication() and len(word) >= 2 * self.__min_stem_len:
            ...
            if word_len % 2 == 0:
                ...
                m_indx = word_len // 2 # 7//2=3
                stem_0 = word[:m_indx]
                ch = word[m_indx]
                stem_1 = word[m_indx + 1:]
                if stem_0 == stem_1 and self.__is_in_lexicon(stem_0):  # xyz-c-xyz
                    if ch == stem_0[-1] and self.__do_r_gemination():  # xyz-z-xyz
                        ...
                        cand = (root, proc)
                        cand_red_pats.append(cand)
                    if ch == stem_1[0] and self.__do_l_gemination():  # xyz-x-xyz
                        b_change = NonChange()
                        ...
                        cand = (root, proc)
                        cand_red_pats.append(cand)
                    if ch != stem_0[-1] and ch != stem_1[0]:  # xyz-c-xyz
        ...
        if self.__do_left_reduplication():
            ...
            for i in range(self.__min_cpy_len, 1 + min(self.__max_cpy_len, len(word) - self.__min_stem_len, len(word) // 2)):
                ...
                if not self.__is_in_lexicon(x):
                    continue
                if self.__find_cand(cand_suf_pats, cond=lambda x: len(x[0]) >= 2 * i):   # 不懂
        ...
        if self.__do_right_reduplication():
            ...
            #-----------------
            for i in range(max(self.__min_stem_len, len(word) - self.__max_cpy_len, len(word) // 2), len(word) - self.__min_cpy_len):  # 不懂 
                x = word[:i]
                ex = word[i:]
        ...
        if self.__do_infixation():
            bad_j = set() # bad_j is to store the j that has been used
            ...
            if len(word) % 2 == 0:
                ...
                if stem_0 == stem_1:
                    ...
                    if self.__do_r_substitution() and stem_0[:-1] == stem_1[:-1] and self.__is_in_lexicon(stem_1):
                        ...
                        pos = StemChangePosition.RIGHT_BOUNDARY # 'r'

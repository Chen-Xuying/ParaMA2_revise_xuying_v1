...
class MorphType(Enum):
    AUTOMATIC = 'aut' # 大写是 constant变量/全局变量————用来统一调用。
    PREFIXATION = 'pref'
    INFIXATION = 'inf'
    SUFFIXATION = 'suf'
    COMPOUNDING = 'comp'
...
class MorphTypology():
    ...
    def __init__(self):
        ...
        morph_features[MorphType.TEMPLATIC] = False      # Not supported yet, e.g. arabic
        morph_features[MorphType.VOWELHARMONY] = False   # Not supported yet, korean?, russian?
...
def get_gold_features(lang):
    ...
    if lang == LanguageCatalog.Akan:
        ...
        typo_feature.set_feature(MorphType.SUFFIXATION, True)
        typo_feature.set_feature(MorphType.PREFIXATION, True) # change to TRUE
        typo_feature.set_feature(MorphType.INFIXATION, False)
        typo_feature.set_feature(MorphType.REDUPLICATION, True)
        #
        typo_feature.set_stem_change_feat(StemChangeType.INS, True)
...
if __name__ == '__main__':
    print(MorphType.INFIXATION,       # MorphType.INFIXATION
          MorphType.INFIXATION.__repr__(),  # <MorphType.INFIXATION: 'inf'>
          MorphType.INFIXATION.name,  # INFIXATION
          MorphType.INFIXATION.value, # inf
          sep='\n')
    pass

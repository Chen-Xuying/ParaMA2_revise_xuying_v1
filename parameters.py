...
@modified content:
    split Hindi into Hindi and HindiRomanized
    hindi case_sensitive = True
    hindiRomanized case_sensitive = False
'''
...
def get_best_parameter(lang):
    ...
    if lang == LanguageCatalog.Akan:
        ...
        params.case_sensitive = True # 只是因为不用lower # 改了。无论roman还是deva，都是insensitive
    elif lang == LanguageCatalog.HindiRomanized:
        params.min_word_freq = 1
        params.min_stem_len = 3
        # maximum & minimum suffix length
        params.max_suf_len = 6
        ...
        params.max_pref_len = 6
        params.min_pref_len = 2
        params.case_sensitive = False # 改了。无论roman还是deva，都是insensitive

...
class DerivationChain():
    ...
    def __cut_circles(self):  # 处理不合理的root？不知道为什么会出现这个情况，但是处理了 root-word-child 中
        for _word, word_node in self.__word_node_indx.items():
            if word_node.is_root(): # 无 parent
                continue
            if word_node.is_leaf(): # 无 child
                continue
            node = word_node
            chain = []
            #-------------debug only-----------
            ...
            found_circle = False
            while node:
                if node.word() in chain_words: # if root in words, then (false) circle found
                    found_circle = True
                    break
                chain_words.add(node.word())
                chain.append(node)  # chain = [word, parent, grandparent, ...]
                node = node.parent()
            if found_circle:
                root_node = chain[0]
                for node in chain[1:]:
                    if len(node.word()) <= len(root_node.word()) and len(node.children()) >= len(root_node.children()):  # 词长<词根长，&&，词.children数>=词根.children数 # 排查错误情况
                        root_node = node
                root_node.parent().remove_child(root_node)  # 排除 root 的parent里有root的情况
                root_node.set_parent(None, None)            # 将 root 的 parent 设置为 None
    ...
    def __get_derivation_chain(self, word):
        ...
        while node: # 用来去重？
            ...
            chain.append(node)
            node = node.parent()
        return chain # [word_node, parent_node, grandparent_node, ...]
